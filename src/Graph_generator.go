...
/*
Ce fichier à pour but de générer un graph de taille donnée en entrés, dans le but d'être utilisés en fichier d'entrée pour Client.go
...
func generator(size int) string { //param : nb de lien voulu
	...
	*/
	...
	for _, letter := range alphabet {
		neighb[letter] = make([]int, len(alphabet)) //Pour chaque clé du map on lui donne comme valeur un tableau d'int de la taille d'alphabet 
	...
	var from, to int//noeud de départ -> noeud d'arrivé -> résultat de la fonction
	var toWrite string 
	n := 0
	reste := alphabet
	for k := 1; k < len(reste); k++ {
		...
		from = reste[n]
		//[0 1 2 3 4]
		if len(neighb[from]) > 3 {//Cette condition est pour garderchaque noeud 3 fois et de passer au prochain apres
			taille = 3
		} else {
		...
		for i := 0; i < taille; i++ {
			...
			to = randValue(neighb[from])
			//fmt.Print(to,"\n")
			if len(neighb[from]) > 1 {
				neighb[from] = remove_element(neighb[from], to)//retrait du voision "to" des voisin de "from"
			}
			if len(neighb[to]) > 1 {
				neighb[to] = remove_element(neighb[to], from)//retrait du meme "from" des voisins de "to" pour ne pas avoir deux poids differents entre les meme noeuds
				//fmt.Print(neighb[to], "\n") 
			}
			...
			toWrite += fmt.Sprintf("%d %d %d\n", to, from, weight) // cette ligne est ajouté pour exprimer un graphe non orienté .
		}
		n += 1
	}
	toWrite += ". . ."
...
func randValue(alphabet []int) int {//je prends une "valeur de noeud " aléatoire dans la slice alphabet
...
func writeGraph(size int, path string) {
	...
	if _, err := f.WriteString(generator(size)); err != nil { //On écrit dans le fichier le résultat de la fonction generator en fonction de la taille de graph voulue, seulement si cette écriture ne produit pas d'erreur
